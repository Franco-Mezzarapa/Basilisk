...
process_cfg* parse_configuration_file() {
    ...
    std::getline(server_configuration, line);
    server_configuration_information->db_ip_address = line.substr(line.find('=') + 1); // Part after '='

    std::getline(server_configuration, line);
    server_configuration_information->db_port = line.substr(line.find('=') + 1); // Part after '='
